...
(function(global, factory) {
    ...
    var extent = function(array, f) {
        ...
        if (f == null) {
            ...
            while (++i < n)
                if ((b = f(array[i], i, array)) != null && b >= b) { a = c = b;
                    break; }
            while (++i < n)
    ...
    var max = function(array, f) {
        ...
        if (f == null) {
            while (++i < n)
                if ((b = array[i]) != null && b >= b) { a = b;
                    break; }
            while (++i < n)
                if ((b = array[i]) != null && b > a) a = b;
        } else {
            while (++i < n)
                if ((b = f(array[i], i, array)) != null && b >= b) { a = b;
                    break; }
            while (++i < n)
    ...
    var nest = function() {
        ...
        return nest = {
            ...
            key: function(d) { keys.push(d);
                return nest; },
            sortKeys: function(order) { sortKeys[keys.length - 1] = order;
                return nest; },
            sortValues: function(order) { sortValues = order;
                return nest; },
            rollup: function(f) { rollup = f;
                return nest; }
        };
    };
        ...
        function parseRows(text, f) {
            ...
            function token() {
                ...
                while (I < N) {
                    ...
                    c = text.charCodeAt(I++);
                    if (c === 10) eol = true; // \n
                    else if (c === 13) { eol = true;
                        if (text.charCodeAt(I) === 10) ++I, ++k; } // \r|\r\n
    ...
    var request = function(url, callback) {
        ...
        "onload" in xhr
            ? xhr.onload = xhr.onerror = xhr.ontimeout = respond : xhr.onreadystatechange = function(o) { xhr.readyState > 3 && respond(o); };
        ...
        if (count) {
            ...
            interval.every = function(step) {
                ...
                return !isFinite(step) || !(step > 0) ? null : !(step > 1) ? interval : interval.filter(field ? function(d) {
                    return field(d) % step === 0; } : function(d) {
    ...
    function deinterpolate(a, b) {
        return (b = Math.log(b / a)) ? function(x) {
            return Math.log(x / a) / b; } : constant$4(b);
    }

    function reinterpolate(a, b) {
        return a < 0 ? function(t) {
            return -Math.pow(-b, t) * Math.pow(-a, 1 - t); } : function(t) {
    ...
    function pow() {
        ...
        function deinterpolate(a, b) {
            return (b = raise(b, exponent) - (a = raise(a, exponent))) ? function(x) {
                return (raise(x, exponent) - a) / b; } : constant$4(b);
        }
    ...
    var selection_style = function(name, value, priority) {
        var node;
        return arguments.length > 1 ? this.each((value == null ? styleRemove : typeof value === "function" ? styleFunction : styleConstant)(name, value, priority == null ? "" : priority)) : window(node = this.node())
    ...
    var interrupt = function(node, name) {
        ...
        for (i in schedules) {
            if ((schedule = schedules[i]).name !== name) { empty = false;
                continue; }
    ...
    var transition_style = function(name, value, priority) {
        ...
        return value == null ? this
            ...
            .on("end.style." + name, styleRemoveEnd(name)) : this.styleTween(name, typeof value === "function" ? styleFunction$1(name, i, tweenValue(this, "style." + name, value)) : styleConstant$1(name, i, value), priority);
    };
    ...
    function axis(orient, scale) {
        ...
        function axis(context) {
            ...
            path
                .attr("d", orient === left || orient == right ? "M" + k * tickSizeOuter + "," + range0 + "H0.5V" + range1 + "H" + k * tickSizeOuter : "M" + range0 + "," + k * tickSizeOuter + "V0.5H" + range1 + "V" + k * tickSizeOuter);
    ...
    function brush$1(dim) {
        ...
        function started() {
            if (exports.event.touches) {
                if (exports.event.changedTouches.length < exports.event.touches.length) return noevent$2(); } else if (touchending) return;
            if (!filter.apply(this, arguments)) return;
        ...
        brush.extent = function(_) {
            return arguments.length ? (extent = typeof _ === "function" ? _ : constant$11([
                [+_[0][0], +_[0][1]],
                [+_[1][0], +_[1][1]]
            ]), brush) : extent;
        };
            ...
            function polygonInside() {
                ...
                for (var i = 0, n = polygon.length; i < n; ++i) {
                    for (var ring = polygon[i], j = 1, m = ring.length, point = ring[0], a0, a1, b0 = point[0], b1 = point[1]; j < m; ++j) {
                        ...
                        if (a1 <= y1) {
                            if (b1 > y1 && (b0 - a0) * (y1 - a1) > (b1 - a1) * (x0 - a0)) ++winding; } else {
    ...
    function graticule() {
        ...
        function lines() {
            return range(ceil(X0 / DX) * DX, X1, DX).map(X)
                ...
                .concat(range(ceil(x0 / dx) * dx, x1, dx).filter(function(x) {
                    return abs(x % DX) > epsilon$4; }).map(x))
                .concat(range(ceil(y0 / dy) * dy, y1, dy).filter(function(y) {
                    return abs(y % DY) > epsilon$4; }).map(y));
        }
    ...
    var clipAntimeridian = clip(
        ...
        clipAntimeridianInterpolate, [-pi$4, -halfPi$3]
    );
    ...
    function projectionMutator(projectAt) {
        var project,
            ...
            x0 = null,
            y0, x1, y1, postclip = identity$7, // clip extent
            delta2 = 0.5,
    ...
    var albersUsa = function() {
        ...
        albersUsa.translate = function(_) {
            ...
            alaskaPoint = alaska
                ...
                .clipExtent([
                    [x - 0.425 * k + epsilon$4, y + 0.120 * k + epsilon$4],
    ...
    function conicConformalRaw(y0, y1) {
        ...
        function project(x, y) {
            if (f > 0) {
                if (y < -halfPi$3 + epsilon$4) y = -halfPi$3 + epsilon$4; } else {
    ...
    var identity$8 = function() {
        var k = 1,
            ...
            x0 = null,
            y0, x1, y1, clip = identity$7, // clip extent
            cache,
            cacheStream,
            projection;
        ...
        return projection = {
            ...
            clipExtent: function(_) {
                return arguments.length ? (clip = _ == null ? (x0 = y0 = x1 = y1 = null, identity$7) : clipExtent(x0 = +_[0][0], y0 = +_[0][1], x1 = +_[1][0], y1 = +_[1][1]), reset()) : x0 == null ? null : [
                    [x0, y0],
                    [x1, y1]
                ];
            },
